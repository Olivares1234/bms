<?php
	...
	foreach ($data['gulod_transactions'] as $gulod_transaction) {
		$data['gulod_beneficiary_balance'] += $gulod_transaction['total'];
	}

	foreach ($data['beneficiaries'] as $beneficiaries) {
		if($data['beneficiary_id'] == $beneficiaries['beneficiary_id']) {
			...
			$beneficiary->balance = $data['gulod_beneficiary_balance'];
