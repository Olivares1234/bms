...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/unavailable_medicine/unavailable_medicine_header.php'); ?>
		...
		<div v-if="unavailable_medicine_list">
			...
			<div class="card px-3 mt-4">
				<div class="float-left mt-4">
					...
					<input @input="searchUnavailableMedicine" type="text" class="form-control form-control-sm float-right col-md-2" v-model="search_unavailable_medicine" placeholder="Search" arial-label="Search" v-if="filter == 'name' || filter == 'category' || filter == 'unit category'">
				...
				<table class="table table-hover table-bordered text-center mt-2">
					...
					<tbody v-if="unavailable_medicines.length > 0">
						<tr v-for="(unavailable_medicine, index) in unavailable_medicines" v-if="index >= startIndex && index < endIndex">
							<td>{{identifyMedicineName(unavailable_medicine.supplier_medicine_id)}}</td>
							<td>{{identifyCategory(unavailable_medicine.supplier_medicine_id)}}</td>
...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/unavailable_medicine/unavailable_medicine_footer.php'); ?>
