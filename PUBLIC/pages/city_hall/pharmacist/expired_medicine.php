...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/expired_medicine/expired_medicine_header.php'); ?>
	...
	<div class="container">
		...
		<div class="card px-3 mt-4">
			<div class="float-left mt-4">
				...
				<input @input="searchExpiredMedicine" type="text" class="form-control form-control-sm float-right col-md-2" v-model="search_expired_medicine" placeholder="Search" arial-label="Search" v-if="filter == 'name' || filter == 'category' || filter == 'unit'">
			...
			<table class="table table-hover table-bordered text-center mt-2">
				...
				<tbody v-if="expired_medicines.length > 0">
					<tr v-for="(expired_medicine, index) in expired_medicines" v-if="index >= startIndex && index < endIndex">
						<td>{{ideitifyMedicineName(expired_medicine.supplier_medicine_id)}}</td>
						<td>{{idetifyCategory(expired_medicine.supplier_medicine_id)}}</td>
						<td>{{identifyUnitCategory(expired_medicine.supplier_medicine_id)}}</td>
						<td>{{expired_medicine.received_quantity}}</td>
						<td>&#8369;{{identifyMedicinePrice(expired_medicine.supplier_medicine_id)}}</td>
						<td>{{expired_medicine.expiration_date}}</td>
					</tr>
				</tbody>
...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/expired_medicine/expired_medicine_footer.php'); ?>