...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/available_medicine/available_medicine_header.php'); ?>
		...
		<div v-if="available_medicine_list">
			...
			<div class="card px-3 mt-4 shadow-nohover">
				<div class="float-left mt-4">
					...
					<input @input="searchAvailableMedicine" type="text" class="form-control form-control-sm float-right col-md-2" v-model="search_available_medicine" placeholder="Search" arial-label="Search" v-if="filter == 'name' || filter == 'category' || filter == 'unit'">
				...
				<table class="table table-hover table-bordered text-center mt-2" v-cloak>
					...
					<tbody v-if="available_medicines.length > 0">
						<tr v-for="(medicine, index) in available_medicines" v-if="index >= startIndex && index < endIndex">
							<td>{{identifyMedicineName(medicine.supplier_medicine_id)}}</td>
							<td>{{identifyCategory(medicine.supplier_medicine_id)}}</td>
							<td>{{identifyUnitCategory(medicine.supplier_medicine_id)}}</td>
							<td>{{medicine.received_quantity}}</td>
							<td>&#8369;{{identifyMedicinePrice(medicine.supplier_medicine_id)}}</td>
						</tr>
					</tbody>
				...
				<div class="mt-1">
					...
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-end">
					    	...
					    	<li class="page-item">
					    		<a class="page-link btn btn-sm bg-primary text-light py-1 px-2" v-for="num in Math.ceil(this.available_medicines.length / this.show_entries) > 3 ? 3 : Math.ceil(this.available_medicines.length / this.show_entries)" @click="pagination(num)" style="display: inline-block">{{num}}
					    		</a>
					    	</li>
					    	<li class="page-item">
...
<?php include(SHARED_PATH . '/city_hall/pharmacist/medicine/available_medicine/available_medicine_footer.php'); ?>