...
<?php include(SHARED_PATH . '/barangay/admin/reports/received_order_reports/received_order_reports_header.php'); ?>

<div id="vue-received_orders-reports" class="mt-4" v-cloak>
	<div class="container">
		...
		<div class="card px-3 pt-2 pb-3 mt-4 shadow-nohover">
			<div class="float-right mt-4">
				<select @change="retrieveReceivedOrderReports(select_reports)" v-model="select_reports" id="select_reports" class="form-control form-control-sm d-inline-block col-md-2 float-right" id="show_entries">
			...
			<div class="box-body chart-responsive" v-cloak>
				...
				<label class="d-inline-block mr-1" for="show_entries">Show </label>
					<select @change="showEntries(show_entries)" v-model="show_entries" class="form-control form-control-sm d-inline-block col-md-2" id="show_entries" style="width: auto;">
						...
						<option :value="received_orders.length">All</option>
					</select>
				...
				<table class="table table-hover table-bordered text-center">
					...
					<tbody v-if="received_orders.length > 0">
						<tr v-for="(received_order, index) in received_orders" v-if="index >= startIndex && index < endIndex">
							<td>{{received_order.Date_Received}}</td>
							<td>{{received_order.Supplier}}</td>
							<td>{{received_order.Total_Order}}</td>
					...
					<tfoot v-if="received_orders.length > 0">
						<tr>
				...
				<div class="mt-1">
					<p class="float-left">Showing {{this.received_orders.length ? startIndex + 1 : 0}} to {{endIndex>this.received_orders.length? this.received_orders.length :endIndex}} of {{received_orders.length}} entries</p>
					...
					<nav aria-label="Page navigation example">
					  <ul class="pagination justify-content-end">
					    ...
					    <li class="page-item"><a class="page-link btn btn-sm bg-primary text-light py-1 px-2" v-for="num in Math.ceil(this.received_orders.length / this.show_entries) > 3 ? 3 : Math.ceil(this.received_orders.length / this.show_entries)" @click="pagination(num)" style="display: inline-block">{{num}}</a></li>
					    <li class="page-item">
...
<?php include(SHARED_PATH . '/barangay/admin/reports/received_order_reports/received_order_reports_footer.php'); ?>