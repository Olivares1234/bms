var vm = new Vue({
	...
	data : {
		...
		view_expired_medicine_details : "View Medicine Details",

		expired_medicine_list : true,
		expired_medicine_details_list : false,
	...
	methods : {
		...
		//search methods
		searchExpiredMedicine : function() {
	    ...
	    retrieveExpiredMedicine : function() {
	    	axios.get(this.urlRoot + this.api + "retrieve_expired_medicine_for_display.php")
	    	.then(function (response) {
	    		vm.expired_medicines = response.data;
	    		console.log(response);
	    	});
		...
		totalPages: function() {
	   	...
	   	totalInventory: function() {
			...
			for(let index = 0; index < this.expired_medicines.length; index++) {
				total += parseInt(this.identifyMedicinePrice(this.expired_medicines[index].purchase_received_details_id)) * parseInt(this.expired_medicines[index].quantity);
			}
