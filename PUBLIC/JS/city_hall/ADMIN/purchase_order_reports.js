var vm = new Vue({
	el : "#vue-purchase_orders-reports",
	data : {
		urlRoot : "/bms/public/",
	...
	methods : {
		download: function() {
			...
			doc.autoTable(columns, vm.purchase_orders);
			doc.save('purchase-order-reports.pdf');
		...
		retrievePurchaseOrderReports : function(search) {
		    var self = this;
		    var sum = 0;
		    var text = "";
		    ...
		    if (this.select_reports == "") {
		    	...
		    	text = "Lifetime Purchase Order"
		    } else if (this.select_reports == "last_week") {
		    	text = "Last Week Purchase Order"
		    } else if (this.select_reports == "this_week") {
		    	text = "This Week Purchase Order"
		    } else if (this.select_reports == "last_month") {
		    	text = "Last Month Purchase Order"
		    } else if (this.select_reports == "this_month") {
		    	text = "This Month Purchase Order"
		    } else if (this.select_reports == "last_year") {
		    	text = "Last Year Purchase Order"
		    } else if (this.select_reports == "this_year") {
		    	text = "This Year Purchase Order"
		    } 
		   
		    axios.post(this.urlRoot + this.api + "retrieve_purchase_order_reports.php?search=" + this.select_reports)
		   	.then(response => {
		    	...
		    	vm.purchase_orders = response.data
				    ...
				    this.myChart = new Chart(ctxChart, {
				        type: 'bar',
				        data: {
				          	labels: vm.purchase_orders.map(item => item.Date_Request),
				          	datasets: [{
				            label: 'Total Order',
				            data: vm.purchase_orders.map(item => item.Total_Order),
				            backgroundColor: this.poolColors(vm.purchase_orders.length),
				            borderColor: "#eee",
				            borderWidth: 2
				        }]
				    },
		   	...
		   	.then(response => {
		    	...
		    	vm.purchase_orders = response.data
				    ...
				    for(var i = 0; i < this.purchase_orders.length; i++) {
				    	sum += parseFloat(this.purchase_orders[i].Total_Order);
				    }
				    ...
				    this.myChart2 = new Chart(ctxChart, {
				        ...
				        data: {
				        	labels: vm.purchase_orders.map(item => item.Date_Request),
				          	datasets: [{
				            label: 'Total Order',
				            data: vm.purchase_orders.map(item => (item.Total_Order / sum * 100).toFixed(2)),
				            backgroundColor: this.poolColors(vm.purchase_orders.length),
				            borderColor: '#eee',
				            borderWidth: 2
				        }]
	...
	computed : {
		totalPages: function() {
	    	return Math.ceil(this.purchase_orders.length / this.show_entries)
	   	},
	   	totalOrder: function() {
	   		var total = 0;
	   		for(var i = 0; i < this.purchase_orders.length; i++) {
	   			total += parseFloat(this.purchase_orders[i].Total_Order);
	   		}
	   		return total;
	   	}
	},
	...
	created() {
		this.retrievePurchaseOrderReports();
